...
def trial(Wji, scale, i, numPairs, pset):
    ...
    return scale, i, numPairs, n_states
...
if __name__ == '__main__':
    ...
    Wji_scale = np.array([0.5, 1.0, 2.0])
    n_trials = 5
    
    def yield_next_task():
        for scale in Wji_scale:
            for numPairs in numPairs_range:
                for i in range(n_trials):
                    means = scale * np.array(Wji_means) * 5 / numPairs  # scale to match the number of pairs (was 5 originally)
                    Wji = network_model.makeWji_all_types(np.random.default_rng(i), numPairs, means, np.abs(means * CV))
                    yield (Wji, scale, i, numPairs, pset)
    ...
    # save data
    data_dir = util.make_data_folder('figures/figure3', name='n_states_vs_n_pairs')
